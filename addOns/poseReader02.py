...
class TPoseReader(object):
    def __init__(self, name, ref, target, numPoses=4, axis='x', outputMult=1.0, createLocs=1):
        '''
        ...
        offsetMtx = mathOps.composeMatrixFromMatrix(target.worldMatrix[0].get() * refInput.get(),
                                                    name = '%s_base2targ_offset_mtx' % name)
        localTargetMtx = mathOps.multiplyMatrices([offsetMtx.outputMatrix, targetInput, refInput],
                                                  name='%s_local_target_mtx' % name)
        ...
        poseAngleNormal.output.connect(projectionCheck.firstTerm)
        projectionCheck.secondTerm.set(0)
...
def buildFromDict(dict):
    pr = TPoseReader(name=dict['name'], ref=pm.PyNode(dict['ref']), target=pm.PyNode(dict['target']),
                     numPoses=len(dict['targets']), axis=dict['axis'], refAxis=dict['axis'], createLocs=0)
