...
class TMouth04(components.TBaseComponent):
    def __init__(self, guide):
        self.guide = guide
        components.TBaseComponent.__init__(self, guide.guide_name, guide.guide_side, guide.guide_index, 'mouth04')
        print 'Created Mouth04 Component: %s' % self.comp_name
    ...
    def addObjects(self, guide):
        ...
        # CONTROLS
        self.root_ctrl = self.addCtrl(shape='squarePoint', size=ctrlSize*4, name=self.getName('root'),
        ...
        self.upperDivs = []
        self.lowerDivs = []
        self.root_srt = dag.addChild(self.rig, 'group', name=self.getName('root_srt'))
    ...
    def addSystems(self):
        # Connect root_srt for global positioning of mouth
        self.base_srt.worldMatrix[0].connect(self.root_srt.offsetParentMatrix)
        self.root_ctrl.ty.connect(self.root_srt.ty)
        self.root_ctrl.tz.connect(self.root_srt.tz)
        self.root_ctrl.rz.connect(self.root_srt.rz)
        ...
        rootOffset = mathOps.divide(self.root_ctrl.tx, crvLen, name=self.getName('root_offset_mult'))

        startParamSum = mathOps.addScalar([startParamMult.outputX, self.guide.root.corner_start.get(), rootOffset.outputX],
        ...
        endParamSum = mathOps.addScalar([endParamMult.outputX, 1.0-self.guide.root.corner_start.get(), rootOffset.outputX],
        ...
        upperMidParamSum = mathOps.addScalar([upperMidParamMult.outputX, startParamAffectsMid.output, rootOffset.outputX,
        ...
        lowerMidParamSum = mathOps.addScalar([lowerMidParamMult.outputX, startParamAffectsMid.output, rootOffset.outputX,
        ...
        base_crv_mtx = mathOps.multiplyMatrices([base_crv_offset.outputMatrix, self.root_srt.worldMatrix[0]],
                                            name=self.getName('base_crv_mtx'))
        base_crv_mtx.matrixSum.connect(self.base_crv.offsetParentMatrix)

        if self.jaw:
            self.root_ctrl.ty.connect(self.jaw_srt.ty)
            self.root_ctrl.tz.connect(self.jaw_srt.tz)
            self.root_ctrl.rz.connect(self.jaw_srt.rz)
        ...
        def _driveCtrl(index, ctrl, prefix='upper'):
            ...
            if 0 < index < len(self.upper_ctrls)-1 and index != midIndex:
                ...
                if index < midIndex:
                    ...
                    twistFactor = (0.5 + (0.5-factor))*-2
                    cornerCtrl = self.upper_ctrls[0]
                else:
                    ...
                    cornerMult = mathOps.multiplyVector(self.upper_ctrls[-1].t,
                                                        ...
                                                        name=self.getName('%s_%s_corner_translate_mult' % (prefix, num)))
                    twistFactor = (0.5 + (0.5-factor))*2
                    cornerCtrl = self.upper_ctrls[-1]
                if prefix == 'upper':
                    midCtrl = self.upper_ctrls[midIndex]
                else:
                    midCtrl = self.lower_ctrls[midIndex-1]

                midTwist = mathOps.convert(midCtrl.rz, twistFactor, name=self.getName('%s_%s_mid_twist_mult' % (prefix, num)))
                midMult = mathOps.multiplyVector(midCtrl.t, (1-factor, 1-factor, 1-factor),
                                                 name=self.getName('%s_%s_mid_translate_mult' % (prefix, num)))
                cornerTwistFactor = (1 - (1-factor)) * -1
                cornerTwist = mathOps.convert(cornerCtrl.rz, cornerTwistFactor,
                                           name=self.getName('%s_%s_corner_twist_mult' % (prefix, num)))
                translateSum = mathOps.addVector([cornerMult.output, midMult.output],
                                                 name=self.getName('%s_%s_translate_sum' % (prefix, num)))
                midTwist.output.connect(translateSum.input3D[2].input3Dy)
                cornerTwist.output.connect(translateSum.input3D[3].input3Dy)
                translateSum.output3Dy.connect(ctrl.getParent().ty)
                translateSum.output3Dz.connect(ctrl.getParent().tz)

            mp = curve.createMotionPathNode(self.base_crv, uValue=paramAttr, wut=2, wuo=self.root_srt,
        ...
        def _driveDiv(div, index, prefix='upper'):
            ...
            if param < 0.5:
                ...
                bias = 1 - ((1-param)*(1-param)*2)
            biasWeight = 0.5
            param = (param*(1-biasWeight))+(bias*biasWeight)
            ...
            baseMp = curve.createMotionPathNode(self.base_crv, uValue=basePoint.result.parameter,
                                                wut=2, wuo=self.root_srt, fractionMode=0,
    ...
    def setColours(self):
        ...
        icon.setColourRGB(self.root_ctrl, colour)
    ...
    def finish(self):
        ...
        masterCtrls = [self.upper_ctrls[0], self.upper_ctrls[-1],
                       self.lower_ctrls[(len(self.upper_ctrls) - 2) / 2],
                       self.upper_ctrls[len(self.upper_ctrls) / 2],
                       self.root_ctrl]
        nodeList = [ctrl for ctrl in self.controls_list if ctrl not in masterCtrls]
        attrList = ['visibility', 'sx', 'sy', 'sz', 'rx', 'ry', 'rz']
        attribute.channelControl(nodeList=nodeList, attrList=attrList)
        attrList = ['visibility', 'sx', 'sy', 'sz', 'rx', 'ry']
        attribute.channelControl(nodeList=masterCtrls, attrList=attrList)
