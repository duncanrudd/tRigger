...
class TFkBlendChainGuide(guide.TGuideBaseComponent):
    def __init__(self, guide_name='', guide_side='C', guide_index=0, num_segments=6, targetChain_01='', targetChain_02='', fromDagNode=0):
        guide.TGuideBaseComponent.__init__(self, guide_name, 'fkBlendChain', guide_side, guide_index, fromDagNode=fromDagNode)
        for param in ['num_segments', 'targetChain_01', 'targetChain_02']:
            self.params.append(param)
        if not fromDagNode:
            attribute.addIntAttr(self.root, 'num_segments', num_segments)
            attribute.addStringAttr(self.root, 'targetChain_01')
            attribute.addStringAttr(self.root, 'targetChain_02')
        ...
        self.targetChain_01 = self.root.targetChain_01.get()
        self.targetChain_02 = self.root.targetChain_02.get()
...
def instantiateFromDagNode(dagNode):
    return TFkBlendChainGuide(dagNode.guide_name.get(),
                         dagNode.guide_side.get(),
                         dagNode.guide_index.get(),
                         dagNode.num_segments.get(),
                         dagNode.targetChain_01.get(),
                         dagNode.targetChain_02.get(),
                         fromDagNode=dagNode)


def buildGuide(**kwargs):
    return TFkBlendChainGuide(**kwargs)
