...
class TShearChainGuide(guide.TGuideBaseComponent):
    def __init__(self, guide_name='', guide_side='C', guide_index=0, num_segments=6, fromDagNode=0):
        guide.TGuideBaseComponent.__init__(self, guide_name, 'shearChain', guide_side, guide_index, fromDagNode=fromDagNode)
        for param in ['num_segments']:
...
def instantiateFromDagNode(dagNode):
    return TShearChainGuide(dagNode.guide_name.get(),
...
def buildGuide(**kwargs):
    return TShearChainGuide(**kwargs)
