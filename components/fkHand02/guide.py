...
class TFkHand02Guide(guide.TGuideBaseComponent):
    def __init__(self, guide_name='', guide_side='C', guide_index=0, num_segments=5, num_digits=4, thumb=1, fromDagNode=0):
        guide.TGuideBaseComponent.__init__(self, guide_name, 'fkHand02', guide_side, guide_index, fromDagNode=fromDagNode)
    ...
    def addLocs(self):
        ...
        pm.delete(crvs)
        self.locs = [self.root]
        self.addGuideLoc(self.getName('master'), transform.getMatrixFromPos((0, 2, 0)), self.root)
...
def instantiateFromDagNode(dagNode):
    return TFkHand02Guide(dagNode.guide_name.get(),
